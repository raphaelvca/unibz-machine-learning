...
After the process is finished you have a `atml_202021` image, that will be the base for your experiments. You can confirm that looking on results of `docker images` command.

### Run the notebooks

Run `make run` (or just `docker-compose up`) to start the jupyter server inside the container (also named `atml_202021`, same as image). Just point your browser to the URL printed on the screen (or just <http://localhost:8888> if you enabled password authentication) and you're ready to play with the book's code!
...
Run `make exec` (or `docker-compose exec atml_202021 bash`) while the server is running to run an additional `bash` shell inside the `atml_202021` container. Now you're inside the environment prepared within the image.
